$("document").ready(function(){
	...
	$("#addBtn").click(function(){
		$.link("mworker/allowance/add?workerId="+workerId+"&guildId="+guildId);
	});
	$("#addForm").validate({
		tipType:"right", 
		ajaxPost:true, 
		...
		callback:function (curform, data) {
			alert(data.msg);
			if(data.code == 1){
				$.link("mworker/"+workerId+"/allowance/list?guildId="+guildId);
			}
		}
	});
	$("#delBtn").click(function(){
		...
		if(checked.length==1){
			if(confirm("您确定要删除吗？")){
				...
				$.ajax({
					url:$.SmartUI.PATH+"mworker/allowance/"+checked.val()+"/del",
					type:"post",
					dataType:"json",
	...
	$("#upBtn").click(function(){
		var checked = $(".list-content table tbody").find("input[type=checkbox]:checked");
		if(checked.length==1){
			$.link("mworker/"+checked.val()+"/allowance/update?guildId="+guildId+"&workerId="+workerId);
		}else{
			alert("请选择一条津贴信息！");
		}
	});
