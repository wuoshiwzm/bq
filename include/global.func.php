...
function get_maincat($catid, $moduleid, $level = -1) {
	...
	$cat = array();
	$result = $db->query("SELECT catid,catname,child,style,linkurl,item,catdir FROM {$db->pre}category WHERE $condition ORDER BY listorder,catid ASC", 'CACHE');
	while($r = $db->fetch_array($result)) {
		$cat[] = $r;
	}
	return $cat;
...
function commentr($username,$a) {
	global $db;
	$r = $db->get_one("SELECT * FROM {$db->pre}member WHERE username='$username'");
	$b = $r['username'];
	if($a)$b = $r['company'];
	return $b;
}
