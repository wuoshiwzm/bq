$("document").ready(function(){
	...
	$("#addBtn").click(function(){
		$.link("zgjz/add?basicId="+basicId);
	});
	$("a[id=backBtn]").click(function(){
		$.link("zgjz/zgjzList?basicId="+basicId);
	});
	...
	$("#addForm").validate({
		...
		ajaxPost:true, 
		dataType:{
			zgjzPhone: /(^[0-9]{3,4}\-[0-9]{7,8}$)|(^[0-9]{7,8}$)|(^\([0-9]{3,4}\)[0-9]{3,8}$)|(^0{0,1}13[0-9]{9}$)|(13\d{9}$)|(15[0135-9]\d{8}$)|(18[267]\d{8}$)/
		}, 
		tipmsg:{
			w:{
				zgjzPhone:"请输入正确的联系方式"
			}
		}, 
	...
	$("#upBtn").click(function(){
		var checked = $(".list-content table tbody").find("input[type=checkbox]:checked");
		if(checked.length==1){
			$.link("zgjz/"+$("#basicId").val()+"/"+checked.val()+"/update");
		}else{
			alert("请选择一个选项！");
		}
	});
	...
	$("#chakan").click(function(){
		var checked = $(".list-content table tbody").find("input[type=checkbox]:checked");
		if(checked.length==1){
			$.link("zgjz/"+$("#basicId").val()+"/"+checked.val()+"/chakan");
		}else{
			alert("请选择一个选项！");
		}
	});
	...
	$("#see").click(function(){
		var checked = $(".list-content table tbody").find("input[type=checkbox]:checked");
		if(checked.length==1){
			$.link("zgjz/"+$("#basicId").val()+"/"+checked.val()+"/info");
		}else{
			alert("请选择一个选项！");
		}
	});
