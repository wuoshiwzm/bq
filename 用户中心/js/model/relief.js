$("document").ready(function(){
	...
	$("#addBtn").click(function(){
		$.link("relief/add?guildId="+guildId);
	});
	$("#addForm").validate({
		tipType:"right", 
		ajaxPost:true, 
		dataType:{
			reliefPace:/^[\w\W]{2,40}$/,
			reliefAppropriation:/^[\w\W]{2,40}$/,
			reliefUse:/^[\w\W]{2,40}$/
		}, 
		tipmsg:{
			w:{
				reliefPace:"请输入2-40个汉字",
				reliefAppropriation:"请输入2-40个汉字",
				reliefUse:"请输入2-40个汉字"
			}
		}, 
		callback:function (curform, data) {
			alert(data.msg);
			if(data.code == 1){
				$.link("relief/reliefList?guildId="+guildId);
			}
		}
	});
	...
	$("#upBtn").click(function(){
		var checked = $(".list-content table tbody").find("input[type=checkbox]:checked");
		if(checked.length==1){
			$.link("relief/"+checked.val()+"/"+$("#guildId").val()+"/update");
		}else{
			alert("请选择一个选项！");
		}
	});
