	...
	case 'add':
		...
		//	if($need_password && !is_payword($_username, $password)) dalert($L['error_payword']);
			...
			if($do->pass($post)) {
                ...
                $post_fileds['bmkssj']=strtotime(str_replace('/','-',$psot_fields['bmkssj']));
				$post_fileds['bmjzsj']=strtotime(str_replace('/','-',$psot_fields['bmjzsj']));
				$post_fileds['tbjzsj']=strtotime(str_replace('/','-',$psot_fields['tbjzsj']));
                 $post_fileds['gsjgsj']=strtotime(str_replace('/','-',$psot_fields['gsjgsj']));
				...
				//dalert($msg, '', $js);
				dheader($MOD['linkurl'].'fbzb.php?step=1&id='.$do->itemid);
			} else {
...
$company=$user['company'];
		...
		if(!$thumb) message('请上传对应资质文件');
			...
			dalert('您的资质已经成功提交，请等待审核',$MOD['linkurl'].'fbzb.php?step=4&id='.$id);
...
if($step==1){
	include template('fbzb2', $module);
}
elseif($step==2){
	if($_userid){
		dheader($MOD['linkurl'].'fbzb.php?step=3');//用户已经登录
	}else{
	include template('fbzb3', $module);
	}
	 
}
elseif($step==3){
	
	include template('fbzb4', $module);
}
elseif($step==4){
	include template('fbzb5', $module);
}
else{
include template('fbzb', $module);
}
?>